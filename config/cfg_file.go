...
/*
	...
	funicton set to provide a simple interface to the user programme.
*/
type Config struct {
...
/*
	Parse reads and parsess a configuration file containing sections and key/value pairs 
	within the sections.  Returns a map of sections (by name) with each entry in the map
	being a map[string]interface{}.  Key/values are converted and stored by the key name
	as either string pointers or float64s.  If the value is quoted, the quotes are removed.
...
/*
	Parse2strs parsees the named file keeping the values as strings, and then we convert each
...
/*
	Internal function to convert an interface into a supported desired value.
*/
